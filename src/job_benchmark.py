...
class JobBenchmark(QtWidgets.QDialog, QtCore.QEvent):
	...
	def __init__(self, runtime, config, main_gui, title="Job Benchmark"):
		...
		if 'mask' in runtime.keys():
			self.mask    = runtime['mask']
		if 'inh5' in runtime.keys():
		...
		try:
			...
			self.config = list(config.values())[0]
			...
			utils.show_message("Job path is invalid (%s) !" % self.savepath)
	...
	def run(self):
		...
		try:
			...
			utils.show_message("Fail to run !", str(err))
			return
		...
		if self.assignments == self.main_gui.namespace['classify_DCPS']:
			...
			utils.show_message("Fail to recognize your assignment (%s) !" % self.assignments)
			return
		# chdir
		prev_dir = os.getcwd()
		try:
			...
			utils.show_message("Check your save path (%s) !" % self.savepath)
		...
		try:
			...
			#self.ui.plainTextEdit.appendPlainText("Task:\n"+cmd+"\n\n")
		except Exception as err:
	...
	def plot(self):
		if self.pobj.poll() is None:
			...
			return
		elif self.pobj.poll() != 0:
			utils.show_message("The Job doesn't finish successfully !")
			return
