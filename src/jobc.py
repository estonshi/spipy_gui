...
class JobCenter(QtWidgets.QDialog, QtCore.QEvent):
			...
			with open(os.path.join(self.rootdir, "JobHub.txt"), 'r') as f:
				for line in f.readlines():
					...
					if "jid" in line:
						...
						if run_view_key in prev_run_view.keys():
	...
	def TableRun_showoff(self, job_type, run_names, datafile, runtime=None):
		...
		for run_name in run_names:
			...
			while True:
				...
				if len(self.jobs) >= 500000 or jid not in self.jobs.keys():
					break
	...
	def get_run_status(self, run_name, module, assg, tag, remarks):
		...
		if run_view_key not in self.run_view.keys():
			job_jss = None
	...
	def packSubmit(self, job_obj):
		...
		# hit-finding
			...
			if self.data_format.lower() in ["cxi", "h5"]:
				...
				if sub_nproc is None:
					utils.show_message("The data file '%s' seems to have some problems. \
						Check whether the 'Data-path in cxi/h5' is correct." %runtime['dataset'])
					return None
			else:
				# TODO
				pass
		...
		# manifold decomposition
			...
			try:
				...
				utils.show_message("Fail to decide mpi rank !", str(err))
				return None
