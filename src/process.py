...
class Process_Settings(QtWidgets.QMainWindow, QtCore.QEvent):
	...
	def initui(self, assignments):
		...
		if assignments == self.namespace['process_HF']:
			...
			self.ui.pushButton_3.clicked.connect(partial(self.npy_file, 3))
			self.ui.pushButton.clicked.connect(partial(self.data_dir, assignments))
			...
			# set triiger
			self.ui.pushButton.clicked.connect(partial(self.data_dir, assignments))
			self.ui.pushButton_2.clicked.connect(partial(self.npy_file, 2))
			self.ui.pushButton_3.clicked.connect(partial(self.npy_file, 3))
			# set/read Process_Settings.param
			if assignments in Process_Settings.param.keys():
				# this assignment has been set, show it
			...
			# show messages
			self.ui.label_temp = QtWidgets.QLabel(self.ui.widget_10)
	...
	def data_dir(self, me):
		'''
		datapath = str(QtWidgets.QFileDialog(self).getExistingDirectory())
		if len(datapath) > 0:
	...
	def cxi_file(self, i):
		...
		if len(cxifile[0])>0 and not os.path.exists(cxifile[0]):
			...
			return
		if i==2:
			self.ui.lineEdit_2.setText(cxifile[0])
		elif i==3:
			self.ui.lineEdit_3.setText(cxifile[0])


	def npy_file(self, i):
		npyfile = QtWidgets.QFileDialog(self).getOpenFileName(None, "Select npy file to open", "", "DATA (*.npy)")
		if len(npyfile[0])>0 and not os.path.exists(npyfile[0]):
			utils.show_message("Can't find file path '%s'" % npyfile[0])
			return
		if i==2:
			self.ui.lineEdit_2.setText(npyfile[0])
		elif i==3:
			self.ui.lineEdit_3.setText(npyfile[0])
	...
	def save(self):
		...
		utils.show_message("Successfully save to %s !" % config_file)
		self.close()
