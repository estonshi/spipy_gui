from spipy import image as spi_image
	...
	* output h5 file format :
	...
	for df in data_files:
		...
		for i in range(num_patterns):
			...
			if aduperphoton > 1:
				adus[i] = aduperphoton
				newpat[i] = spi_image.preprocess._transfer(np.array([pat]), 0, aduperphoton, force_poisson)[0]
			else:
				...
				if adu < 1.0:
					adu = 1.0
				adus[i] = adu
				newpat[i] = spi_image.preprocess._transfer(np.array([pat]), 0, adu, force_poisson)[0]
		...
		# write to file
		if mpi_rank == 0:
			# save all adus and patterns from only rank 0
			# print status
			...
			for tmp in adus_gather:
				adus[list(tmp.keys())] = list(tmp.values())
			# here patterns are not gatherred, we only save those from rank 0
			for k, v in newpat.items():
				newpats[k] = v
		...
		if mpi_rank > 0:
			# save patterns from other ranks
			...
			while savef is None:
				try:
					savef = h5py.File(save_file, "a")
				except:
					savef = None

			for k, v in newpat.items():
				savef["PhotonCount/data"][k] = v
			savef.close()
