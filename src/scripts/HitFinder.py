	...
	if sec == "darkcal":
		...
		if mpi_rank == 0:
			...
			for ind, df in enumerate(data_files):
				...
				fp.close()
			meanBg = meanBg / float(num_patterns)
		...
		if mpi_rank == 0:
			...
			if os.path.exists(mask_file):
				...
				user_mask = user_mask.astype(int)
			else:
				user_mask = None
		...
		if radii_range[2] < 0 or radii_range[3] < 0 or radii_range[3] <= radii_range[2]:
			inner_shell = image.radp.circle(2, radii_range[2]) + np.array(radii_range[:2]).astype(int)
			outer_shell = image.radp.circle(2, radii_range[3]) + np.array(radii_range[:2]).astype(int)
		...
		for df in data_files:
			...
			if mpi_rank == 0:
				...
				for tmp in chi_score_gather:
					chi_score[list(tmp.keys())] = list(tmp.values())
				for tmp in label_gather:
					label[list(tmp.keys())] = list(tmp.values())
