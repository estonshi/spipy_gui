...
if __name__ == '__main__':
	...
	* runtime.json should contain : 
		method : simulation method, "fft" or "atomic"
	...
	* output h5 file format :
	...
	config_param = {'detd' : config.getfloat(sec, "detd"), \
					'lambda' : config.getfloat(sec, "lambda"), \
					'detsize' : config.getint(sec, "det_size"), \
					'pixsize' : config.getfloat(sec, "pix_size"), \
					'stoprad' : config.getint(sec, "beam_stop"), \
					'polarization' : 'x', \
					'num_data' : num_pattern, \
					'fluence' : config.getfloat(sec, "fluence") * 1e14, \
					'phy.scatter_factor' : False, \
					'phy.ram_first' : True, 'phy.projection' : False, \
					'photons' : False, 'absorption' : False }
	if config.getint(sec, "photons") > 0:
		config_param['photons'] = True
	if config.getint(sec, "absorption") > 0:
		config_param['absorption'] = True
	if method == sim_adu.simulation.sim_methods[0]:
		# method is atom scatering
		if config.getint(sec, "scatter_f") > 0:
			config_param['phy.scatter_factor'] = True
		if config.getint(sec, "projection") > 0:
			config_param['phy.projection'] = True

	# mask
	if os.path.isfile(config.get(sec, "mask")):
		mask = np.load(config.get(sec, "mask"))
		print("- Load mask file.")
		if mask.shape[0] != mask.shape[1] or mask.shape[0] != config_param['detsize']:
	...
	# simulation
	sim_adu.go_magic(method=method, save_dir=savepath, pdb_file=pdbfile, param=config_param, \
	...
	# MPI.Finalize() has been called in sim_adu.go_magic()
