...
class jobView(QtWidgets.QMainWindow, QtCore.QEvent):
	...
	def load_jobs(self, refresh_main = False):
		...
		for runview, jid in run_view.items():
			...
			if run_name not in self.run_job.keys():
				self.run_job[run_name] = [jid]
			else:
				self.run_job[run_name].append(jid)
		# cmd history tag
		tree_item = QtWidgets.QTreeWidgetItem(self.ui.treeWidget)
		...
		for run_name, jid_list in self.run_job.items():
			...
			for jid in jid_list:
				...
				# Item
				tmp = QtWidgets.QTreeWidgetItem(tree_item)
				tmp.setText(0, self.mainwindow.JobCenter.get_runviewkey(jid))
				tmp.setText(1, status)
				tmp.setBackground(1, QtGui.QBrush(QtGui.QColor(color[0], color[1], color[2], 127)))
				# add
		...
		if action == a1:
			...
			if re == 1:
				for si in selected:
					...
					if job_status is None or job_status == self.mainwindow.JobCenter.RUN or job_status == self.mainwindow.JobCenter.SUB:
						utils.show_message("The status of job %s is %s, cannot be deleted !" % (str(si.text(0)), str(job_status)) )
						success = 0
						continue
					else:
						try:
						...
						if success:
							utils.print2projectLog(self.mainwindow.dirname, "Delete project %s" % job_path)
			else:
				success = 0

		elif action == a2:
			warning = "Are your sure to kill all of the jobs ? "
			choose_data = ""
			for si in selected:
				choose_data = choose_data + str(si.text(0)) + '\n'
			re = utils.show_warning(warning, choose_data)
			if re == 1:
				for si in selected:
					jid = self.mainwindow.JobCenter.run_view[str(si.text(0))]
					job_status = self.mainwindow.JobCenter.get_run_status_2(jid)
					if job_status is None or job_status != self.mainwindow.JobCenter.RUN:
						utils.show_warning("The job is not running, cannot be killed !")
						success = 0
						continue
					else:
						tmp = self.mainwindow.JobCenter.kill_job(jid)
						if tmp[0] == 1:
							utils.print2projectLog(self.mainwindow.dirname, "Terminate job %s" % str(si.text(0)))
							utils.show_message("Job %s is successfully terminated" % str(si.text(0)))
						else:
							utils.print2projectLog(self.mainwindow.dirname, "Fail to terminate job %d" % str(si.text(0)))
							utils.show_message("**FAIL** to terminate job !" % str(si.text(0)))
							success = 0
							continue
			else:
				success = 0
