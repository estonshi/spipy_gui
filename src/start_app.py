...
class SPIPY_START(QtWidgets.QMainWindow, QtCore.QEvent):
	...
	def __init__(self, parent=None):
		...
		self.ui.pushButton.clicked.connect(self.workdir)
		self.ui.pushButton_2.clicked.connect(self.OK)
		self.ui.pushButton_3.clicked.connect(self.choose_datadir)
		# other attributes
		self.dirname = None
		self.datadir = None
		self.jss = None
		...
		self.monitor_time = self.namespace['monitor_time']
		# set jss ui
	...
	def choose_datadir(self):
		dirname = str(QtWidgets.QFileDialog(self).getExistingDirectory())
	...
	def makedirs(self):
		...
		for n in self.namespace['project_structure']:
			...
			if not os.path.exists(path2):
				os.mkdir(path2)
			if '%s_assignments' % n.lower() in self.namespace.keys():
